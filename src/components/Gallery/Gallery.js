...
const Gallery = ({ input, pageNumber }) => {
	...
	const getPhotos = async () => {
		try {
			fetch(
				`https://api.flickr.com/services/rest/?method=flickr.photos.getRecent&api_key=d1554218567bfe198c78fc3c2b68b48b&per_page=12&page=${pageNumber}`
			)
				.then(res => res.text())
				.then(resText => {
	...
	const getInputPhotos = async input => {
		try {
			fetch(
				`https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=d1554218567bfe198c78fc3c2b68b48b&per_page=12&page=${pageNumber}&text=${input}`
			)
				.then(res => res.text())
				.then(resText => {
					parseString(resText, (err, data) => {
